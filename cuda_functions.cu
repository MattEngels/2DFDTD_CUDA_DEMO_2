...
__global__ void EzUpdate2D_Kernel(double *dEz, double *dCezh, double *dCeze, float *dImEz, double *dHx, double *dHy, int DIM, int time)
{
	...
	else if ((col == src_pos_x) && (row == src_pos_y))	dEz[offset] = cos(2 * PI*time / 200);
...
void update_all_fields_CUDA()
{
	...
	EzUpdate2D_Kernel << <BLK, THD >> >(dev_ez, dev_cezh, dev_ceze, dev_ez_float, dev_hx, dev_hy, g->sizeX, g->time);
	...
	//dev_ez[src_pos_x + src_pos_y * M] = ezIncCos(g->time);	// update the source
}
